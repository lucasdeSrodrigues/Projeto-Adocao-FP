...
while True:
    def relatorio_abrigo():
        print("\n" + "="*50)
    print("RELATÓRIO DO ABRIGO")
    print("="*50)
    ...
    for animal in animais:
        if animal['especie'].lower() == 'cachorro':
            cachorros += 1
        elif animal['especie'].lower() == 'gato':
            gatos += 1

        if animal['saúde'].lower() == 'saudavel':
            saudaveis += 1

    print(f"Total de animais cadastrados: {total_animais}")
    print(f"Total de cuidados registrados: {total_cuidados}")
    print(f"Total de cachorros: {cachorros}")
    print(f"Total de gatos: {gatos}")
    print(f"Animais saudáveis: {saudaveis}")
